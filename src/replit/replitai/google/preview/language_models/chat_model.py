...
from replitai.chat_model import ChatModel as ReplitChatModel, ChatModelResponse
from replitai.structs import ChatSession as ReplitChatSession, ChatExample as ReplitChatExample, ChatMessage as ReplitChatMessage
...
class ChatSession:
  ...
  async def async_send_message(self, message: str, **kwargs):
    ...
    predictParams = dict(**self.parameters, **kwargs)
    session = self.get_chat_session()
    response = await self.underlying_model.apredict(
  ...
  def send_message_stream(self, message: str, **kwargs):
    ...
    response = self.underlying_model.predict_stream(
        [session], ready_parameters(predictParams))
    message = ""
  ...
  async def async_send_message_stream(self, message: str, **kwargs):
    ...
    predictParams = dict(**self.parameters, **kwargs)
    session = self.get_chat_session()
    response = self.underlying_model.apredict_stream(
        [session], ready_parameters(predictParams))
    message = ""
  ...
  def get_chat_session(self):
    examples = [
        self.__build_replit_chat_example_from_io(x) for x in self.examples
    ]
    msgs = [
    ...
    return ReplitChatSession(context=self.context,
                             examples=examples,
                             messages=msgs)

  @staticmethod
  def __build_replit_chat_example_from_io(io: InputOutputTextPair):
    return ReplitChatExample(input=ReplitChatMessage(content=io.input_text),
  ...
  def __ready_response(self,
                       response: ChatModelResponse) -> TextGenerationResponse:
    """
...
class ChatModel:
  ...
  def start_chat(
      ...
      context: Optional[str] = "",
      examples: Optional[List[InputOutputTextPair]] = None,
      message_history: Optional[List[ChatMessage]] = None) -> ChatSession:
    chat_session = ChatSession(self.underlying_model, context, examples or [],
                               message_history or [])
    return chat_session
